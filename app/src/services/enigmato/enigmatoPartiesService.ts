...
export const fetchParticipantsScoresAsync = async (id_party: number, navigate: ReturnType<typeof useNavigate>) => {
  try {
    const response = await axios.get(`${API_ENIGMATO_PARTICIPANTS_URL}/${id_party}/scores`, {
      withCredentials: true,
    });
    return response.data;
  } catch (error) {
...
export const fetchParticipantByIdAsync = async (id_party: number, id_user: number, navigate: ReturnType<typeof useNavigate>) => {
  try {
    const response = await axios.get(`${API_ENIGMATO_PARTICIPANTS_URL}/${id_party}/user/${id_user}`, {
      withCredentials: true,
    });
    return response.data;
  } catch (error) {
